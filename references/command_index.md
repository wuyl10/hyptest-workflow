...
本文是 `hyptest-workflow` 的常用命令索引。README 只保留最高频入口；完整命令由
`$HYPTEST_WORKFLOW_SKILL_HOME/scripts/list_skill_commands.py --markdown` 生成到这里。
...
```bash
python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/validate_task_request.py --repo-root $HYPTEST_HOME --test-point-file test_point/<file>.md ...
```
...
先跑 `python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/resolve_spec_profile.py --spec-profile <spec_profile>`。
...
- `repo-health`: 按 profile、skill、repo、CLI、lint baseline、环境和 self-check 分组输出健康报告。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/doctor.py --repo-root $HYPTEST_HOME --pre-submit --strict --spec-profile <spec_profile>
  ```
...
- `target-module-check`: Bug-hunt prerequisite: validate target_module name with exact/CamelCase/Levenshtein fuzzy matching against the Nanhu RTL source tree before reading source.
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/check_target_module.py --module <target_module>
  ```
...
- `nongate-summary`: Compact Spike-nongate keyword summary for a target_module, driven by the profile's hyptest-nongate-keywords block. Used at Workflow step 4 to avoid reading the full profile markdown.
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/query_spec_profile.py --spec-profile <spec_profile> --nongate-summary --match-module <target_module> --json
  ```
...
- `manual-reference-topic`: Workflow step 16 pre-check: decide whether to append a new Manual_Reference entry by consulting profile nongate keywords, memory confirmed entries, and unresolved MR entries. Returns verdict = profile_covered / memory_confirmed / manual_reference_open / new_entry_needed.
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/check_manual_reference_topic.py --repo-root $HYPTEST_HOME --case <case_name> --module <target_module> --topic <kw1> --topic <kw2> --spec-profile <spec_profile>
  ```

- `doctor-all`: 运行 doctor 分组检查，包含 Spike 和 LinkNan 环境检查。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/doctor.py --repo-root $HYPTEST_HOME --platform all --pre-submit --spec-profile <spec_profile>
  ```
...
- `similar-cases`: 写新 case 前搜索已有 ai/manual case 源文件。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/find_similar_cases.py --repo-root $HYPTEST_HOME --query '<scenario terms>' --limit 5 --explain-score
  ```

- `repo-evidence-index`: 构建或复用全仓 case、test_point、注册状态证据索引缓存。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/repo_evidence_index.py --repo-root $HYPTEST_HOME --query '<scenario terms>' --json
  ```
...
- `case-preflight-pack`: 写 case 前聚合任务、规格/平台口径、环境、repo 快照和相似 case reading pack；覆盖范围可按 task_mode 自动推导。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/case_preflight_pack.py --repo-root $HYPTEST_HOME --test-point-file <test_point_file> --platform spike --spec-profile <spec_profile> --task-mode new-case-only --new-case-count 1 --query '<scenario terms>' --md-out $HYPTEST_HOME/.hyptest_workflow_skill/reports/case_preflight.md --json-out $HYPTEST_HOME/.hyptest_workflow_skill/reports/case_preflight.json
  ```

- `case-skeleton`: 从 preflight 证据生成保守 C 骨架，不裁决 case 语义。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/make_case_skeleton.py --case <case_name> --preflight-json $HYPTEST_HOME/.hyptest_workflow_skill/reports/case_preflight.json --test-point-id <PnX>
  ```

- `case-name-suggest`: 根据 preflight/test_point 术语建议 case 名，并做全仓精确/相似命名冲突检查。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/suggest_case_name.py --repo-root $HYPTEST_HOME --preflight-json $HYPTEST_HOME/.hyptest_workflow_skill/reports/case_preflight.json --prefix ai_micro --json
  ```

- `case-uniqueness`: 写新 case 前检查精确函数名唯一性，默认复用 repo evidence cache，避免每次 rg 冷扫。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/check_case_uniqueness.py --repo-root $HYPTEST_HOME --case <case_name> --expect absent --json
  ```
...
- `repo-snapshot`: 只读汇总 case 源、注册状态、test_point 覆盖、产物和最新日志。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/repo_snapshot.py --repo-root $HYPTEST_HOME
  ```

- `baseline-diff`: 接受新 baseline 前查看新增/删除的 lint baseline 问题。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/case_lint_baseline_diff.py --old <old_baseline.json> --new <new_baseline.json>
  ```
...
- `profile-query`: 按地址汇总 PMA/PBMT/MMIO profile 规则。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/query_spec_profile.py --spec-profile <spec_profile> --address <pa> --summary
  ```
...
- `new-profile`: 基于模板创建新的 spec profile 骨架并写入注册表。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/new_spec_profile.py --name <profile_name> --title '<project/core title>' --update-registry
  ```

- `profile-decision`: 只输出匹配地址的 profile default_decision。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/query_spec_profile.py --spec-profile <spec_profile> --address <pa> --decision-only
  ```

- `reason-code`: 根据失败现象给出候选 reason_code。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/suggest_reason_code.py --symptom '<failure symptom>'
  ```

- `reason-code-eval`: 检查失败现象到 reason_code 建议的 eval 样例。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/eval_reason_code_suggestions.py
  ```
...
- `triage-handoff`: 生成 workflow 交给 failure-triage 深入分析的交接卡片；可附带可选波形上下文，由 failure-triage 决定是否调用 waveform-debug。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/make_triage_handoff.py --log-file <log> --platform linknan --spec-profile <spec_profile> --json
  ```
...
- `manifest-update`: 新增或删除脚本后刷新 assets/script_manifest.json。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/update_script_manifest.py --write
  ```

- `resource-index-update`: 刷新 resource_index.md 中的资源覆盖生成块。

  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/update_resource_index.py --write
  ```
...
- `listed-command-help`: 检查 README/命令清单里的脚本仍然支持 --help。
  ...
  ```bash
  python3 $HYPTEST_WORKFLOW_SKILL_HOME/scripts/eval_listed_commands_help.py
  ```
